...
import { bodySchema, type BetaSignupBody } from "./schema.js";
import { insertBetaSignup } from "./repository.js";

export const betaRoutes: FastifyPluginAsync = async (app: FastifyInstance) => {
  app.post<{ Body: BetaSignupBody }>(
    "/api/beta",
    ...
    async (request, reply) => {
      ...
      try {
        await insertBetaSignup(request.body);
        return reply.code(201).send({ status: "ok" });
      } catch (err) {
